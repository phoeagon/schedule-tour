...
var GoogleMap = (function() {
    var newLonLat = function(lon, lat) {
...
var OpenMap = (function() {
    ...
    var newMap = function(domid) {
        map = new OpenLayers.Map(domid);
        //google map layers
        map.addLayers([
    ...
    var initMarkerLayer = function() {
        markers = new OpenLayers.Layer.Markers( "Markers" );
        map.addLayer(markers);
        return markers;
    }
    var newMarker = function(lonLat) {
        var marker = new OpenLayers.Marker(lonLat);
        marker.events.register('click', null, function(){console.log('a');});
        markers.addMarker(marker);
        return marker;
    }
    var removeMarker = function(marker) {
        if (markers && marker) {
            markers.removeMaker(maker);
        }
    }
...
var DirectionService = (function() {
    var route = function(request, callback) {
        directionsService.route(request, function(response, status) {
            var ret = [];
            try {
                ret = response.routes[0].legs[0].steps;
            } catch (e) {
                ret = [];
            }
            callback(ret);
...
var ScheduleTour = (function() {
    ...
    var initMap = function(mapDOM) {
        ...
        map.zoomToMaxExtent();
        var marker = Map.newMarker(Map.newLonLat(121, 31));
        return;
...
/*
        ...
        */
        e.marker.events.remove('click');
        e.marker.events.register('click', null, function() {
    ...
    var newMarkerToEvent = function(map, e) {
        ...
        var p = Map.newLonLat(e.position[1], e.position[0]);
...
/* TODO: translate this into openlayers one
        geocoder.geocode(
            {
                latLng: latLng
            },
        ...
        */
        ...
        $('#title').val('');
        $('#description').val('');
        $('#newLat').val(lonLat.lat());
        $('#newLng').val(lonLat.lng());
